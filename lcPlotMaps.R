landcoverPlotMaps <- function(output.path, output.name, countries.SPDF,
                        countries.bg = "white", 
                        countries.border = "gray70",
                        map.raster, map.title, 
                        scalebar.dist=1000,northarrow.size=3,
                        northarrow.topoffset=1, colors, legend.cols = 2,
                        legend.lables, pdfGrid = c(5,2), region.ts) {
        ...
        plot(countries.SPDF, lwd=0.2, border=countries.bg,col=countries.bg, 
             ...
             ylim=c(ymin(map.raster) - 7, ymax(map.raster) - 1),axes = F)
        ...
        text(xmin(map.raster) + 0.5, ymin(map.raster) - 3,
             ...
             Data Source: UMD/GADM", adj=c(0,0), cex=0.7, col="gray18")
        ...
        legend("bottomright",  legend = legend.lables, 
                                 fill = colors,ncol=legend.cols,cex = 0.7)
        north.coord <- c(xmin(map.raster), ymax(map.raster) - northarrow.topoffset)
